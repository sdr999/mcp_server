...
import { Sliders, Search, CheckCircle2, AlertTriangle, Code2, Clock, Copy, Check, Terminal, Cpu, Radio, Shield, Zap, Layers, Box } from 'lucide-react';
      ...
      <div className="hud-panel" style={{ padding: '1rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{
            ...
            <Sliders style={{ width: '1.5rem', height: '1.5rem' }} />
          </div>
          <div>
            <h3 className="font-title" style={{ fontSize: '1.25rem', color: '#ffffff', margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              MODULE FITTING BAY
              <span style={{ fontSize: '0.75rem', padding: '0.15rem 0.5rem', background: 'rgba(56, 189, 248, 0.15)', border: '1px solid #38bdf8', color: '#38bdf8', borderRadius: '0.25rem' }}>
                {tools.length} MODULES FITTED
              </span>
            </h3>
            <p className="font-mono" style={{ fontSize: '0.75rem', color: '#94a3b8', margin: 0, marginTop: '0.2rem' }}>
              SELECT SHIP MODULE • ADJUST PARAMETERS • ONLINE MODULE
            </p>
          </div>
        </div>
      </div>

      {/* Main Grid: Fitting Slots (Left) & Module Station (Right) */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '1.5rem' }}>
        {/* Left Column: Fitted Modules List */}
        <div style={{ gridColumn: 'span 5', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {/* Search Box */}
          <div style={{ position: 'relative' }}>
            <Search style={{ position: 'absolute', left: '0.875rem', top: '50%', transform: 'translateY(-50%)', width: '1rem', height: '1rem', color: '#e5a93b' }} />
            <input
              type="text"
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              placeholder="Search ship modules & subroutines..."
              className="font-mono"
              style={{
                width: '100%',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem', maxHeight: '650px', overflowY: 'auto', paddingRight: '0.25rem' }}>
            {loading ? (
              <div className="font-mono" style={{ textAlign: 'center', padding: '3rem 0', color: '#64748b', fontSize: '0.8rem' }}>
                SCANNING FITTING SLOTS...
              </div>
            ) : filteredTools.length === 0 ? (
              <div className="font-mono" style={{ textAlign: 'center', padding: '3rem 0', color: '#64748b', fontSize: '0.8rem' }}>
                NO MATCHING SHIP MODULES FOUND
              </div>
            ) : (
              filteredTools.map(tool => {
                ...
                return (
                  <div
                    ...
                    className="eve-module-card"
                    style={{
                      borderColor: isSelected ? '#e5a93b' : 'rgba(42, 62, 102, 0.8)',
                      boxShadow: isSelected ? '0 0 15px rgba(229, 169, 59, 0.35)' : undefined,
                      background: isSelected ? 'linear-gradient(180deg, #152238 0%, #0d1524 100%)' : undefined
                    }}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.35rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <Box style={{ width: '1rem', height: '1rem', color: isSelected ? '#e5a93b' : '#38bdf8' }} />
                        <h4 className="font-title" style={{ fontSize: '0.95rem', color: '#ffffff', margin: 0 }}>
                          {tool.name}
                        </h4>
                      </div>
                      <span className="font-mono" style={{ fontSize: '10px', color: '#38bdf8', background: 'rgba(56, 189, 248, 0.1)', padding: '0.15rem 0.4rem', borderRadius: '0.25rem', border: '1px solid rgba(56, 189, 248, 0.3)' }}>
                        ONLINE
                      </span>
                    </div>
                    <p style={{ fontSize: '0.75rem', color: '#94a3b8', margin: 0, marginTop: '0.2rem', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                      {tool.description || 'Specialized citadel module protocol.'}
                    </p>
                  </div>
                );
              })
        ...
        {/* Right Column: Module Execution Station */}
        <div className="hud-panel" style={{ gridColumn: 'span 7', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          {selectedTool ? (
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(42, 62, 102, 0.8)', paddingBottom: '0.85rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: '0.75rem', color: '#94a3b8', margin: 0, marginTop: '0.2rem' }}>
                      {selectedTool.description || 'Activate module protocol on MCP Citadel.'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div style={{ backgroundColor: '#060911', border: '1px solid rgba(42, 62, 102, 0.8)', borderRadius: '0.375rem', padding: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  <span className="font-title" style={{ fontSize: '0.8rem', color: '#e5a93b' }}>
              ...
              {/* Parameter Form */}
              <div style={{ backgroundColor: '#060911', border: '1px solid rgba(42, 62, 102, 0.8)', borderRadius: '0.375rem', padding: '1.25rem' }}>
                <SchemaForm
              ...
              {executionResult && (
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                    <h4 className="font-title" style={{ fontSize: '1rem', color: executionResult.success ? '#34d399' : '#fb7185', margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      {executionResult.success ? <CheckCircle2 style={{ width: '1.15rem', height: '1.15rem' }} /> : <AlertTriangle style={{ width: '1.15rem', height: '1.15rem' }} />}
                      {executionResult.success ? 'MODULE ACTIVATION SUCCESSFUL' : 'MODULE OVERLOAD / ERROR'}
                    </h4>
                    {executionDuration && (
                      <span className="font-mono" style={{ fontSize: '0.75rem', color: '#e5a93b', background: '#090e18', padding: '0.2rem 0.5rem', borderRadius: '0.25rem', border: '1px solid rgba(42, 62, 102, 0.8)' }}>
            ...
            <div className="font-title" style={{ textAlign: 'center', padding: '6rem 0', color: '#64748b' }}>
              <Layers style={{ width: '2.5rem', height: '2.5rem', margin: '0 auto 0.75rem auto', color: '#1e2c45' }} />
              SELECT A FITTED MODULE ON THE LEFT TO ACTIVATE
            </div>
          )}
        </div>
      </div>
